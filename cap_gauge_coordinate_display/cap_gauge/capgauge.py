...
dll = windll.LoadLibrary("nicaiu.dll")   # NI's DAQmx driver
...
class CompositeCapgauge(object):
    ...
    def __init__(self, channels, config, name="A Composite Capgauge Sensor"):
        ...
        returnValue = dll.DAQmxCreateTask(self.taskName, byref(self.taskHandle))
        if returnValue < 0:
            errMsg = self.__getDAQmxError__(returnValue)
            raise CapgaugeException('dll.DAQmxCreateTask(self.taskName, byref(self.taskHandle))', \
        ...
        ### sample size and sample rates ###
        if self.parameters.has_key('samples'):
            self.samples = self.parameters['samples']
        else:
            self.samples = 1000

        if self.parameters.has_key('samplingRate'):
            self.sampling_rate = self.parameters['samplingRate']
        else:
            self.sampling_rate = 1000

        ### create channels attach to the task ###
        returnValue = dll.DAQmxCreateAIVoltageChan(self.taskHandle, physicalChannels, channelName, terminalConfig, minVal, maxVal, units, customScaleName)
        if returnValue < 0:
            errMsg = self.__getDAQmxError__(returnValue)
            raise CapgaugeException('dll.DAQmxCreateAIVoltageChan(self.taskHandle, physicalChannels, channelName, terminalConfig, minVal, maxVal, units, customScaleName)', \
    ...
    def __del__(self):
        ...
        @Purpose:
            destructor
        """
        returnValue = dll.DAQmxClearTask(self.taskHandle)
    ...
    def __getDAQmxError__(self, errorCode):
        ...
        returnValue = dll.DAQmxGetErrorString(errorCode, string_buffer, c_uint32(buffer_size))
        if returnValue != 0:
            raise CapgaugeException('returnValue = dll.DAQmxGetErrorString(errorCode, string_buffer, c_uint32(buffer_size))', \
    ...
    def getSampleRates(self):
        ...
        """
        retval = c_double(0)
        returnValue = dll.DAQmxGetSampClkRate(self.taskHandle, byref(retval))
        return retval.value
    ...
    def samplesData(self, number, rates=0.0):
        ...
        if rates != 0.0:
            ...
            returnValue = dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(rates), \
                self.DAQmxConstants['DAQmx_Val_Rising'], self.DAQmxConstants['DAQmx_Val_FiniteSamps'], c_uint64(number))
            if returnValue < 0:
                errMsg = self.__getDAQmxError__(returnValue)
                raise CapgaugeException("dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(rates), \
                    self.DAQmxConstants['DAQmx_Val_Rising'], self.DAQmxConstants['DAQmx_Val_FiniteSamps'], c_uint64(number))", \
                    "ERROR: calling DAQmx function: %s FAILED. \n Error message: %s" % ('DAQmxCreateAIVoltageChan', errMsg))
            ############################################
        ...
        returnValue = dll.DAQmxReadAnalogF64(self.taskHandle, number, timeout, fillMode, sample_value, number*len(self.channels), byref(sampsRead), reserved)
        if returnValue < 0:
            errMsg = self.__getDAQmxError__(returnValue)
            raise CapgaugeException("dll.DAQmxReadAnalogF64(self.taskHandle, number, timeout, fillMode, sample_value, number, byref(sampsRead), reserved)", \
                "ERROR: calling DAQmx function: %s FAILED. \n Error message: %s" % ('DAQmxCreateAIVoltageChan', errMsg))
        ############################################

        if sampsRead.value != number:      # raise exceptions
            raise CapgaugeException("returnValue = dll.DAQmxReadAnalogF64(self.taskHandle, number, timeout, fillMode, sample_value, number, byref(sampsRead), reserved)", \
        ...
        if rates != 0.0:        # reset
            ...
            returnValue = dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(0.0), \
                self.DAQmxConstants['DAQmx_Val_Rising'], self.DAQmxConstants['DAQmx_Val_FiniteSamps'], c_uint64(number))
            if returnValue < 0:
                errMsg = self.__getDAQmxError__(returnValue)
                raise CapgaugeException("dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(0.0), \
                    self.DAQmxConstants['DAQmx_Val_Rising'], self.DAQmxConstants['DAQmx_Val_FiniteSamps'], c_uint64(number))", \
                    "ERROR: calling DAQmx function: %s FAILED. \n Error message: %s" % ('DAQmxCreateAIVoltageChan', errMsg))
            ###############################################
        """
    ...
    def samplesStats(self, sampleSize, rates):
        """
        @Purpose:
            sample the CompositeCapgauge data and calculates statistics (average, standard deviations, peak to valley)
        @Inputs:
            (int) sampleSize = number of samples to take
            (float) rates = sampling rates in samples/seconds
        @Outputs:
            (list) [average_array, std_array, ptv_array, rawData], where:
                (numpy.array) average_array = [chan0_avg, chan1_avg, ...]
                (numpy.array) std_array = [chan0_std, chan1_std, ...]
                (numpy.array) ptv_array = [chan0_ptv, chan1_ptv, ...]
                (list) data_in_channel = [[chan0_raw], [chan1_raw], ...]
        """
        rawData = self.samplesData(sampleSize, rates)
        data_in_channel = [[] for i in xrange(len(self.channels))]
        if self.isDataInterleaved():            # if interleaved
            ...
            for c in range(len(self.channels)):
                data_in_channel[c] = rawData[c*sampleSize:(c+1)*sampleSize]

        avg_array = numpy.zeros(len(self.channels))
        std_array = numpy.zeros(len(self.channels))
        ptv_array = numpy.zeros(len(self.channels))
        for chan in xrange(len(self.channels)):
            avg_array[chan] = numpy.mean(data_in_channel[chan])
            std_array[chan] = numpy.std(data_in_channel[chan])
            ptv_array[chan] = numpy.max(data_in_channel[chan]) - numpy.min(data_in_channel[chan])
        return [avg_array, std_array, ptv_array, data_in_channel]

    def readPositions(self, slope=25, piston=0):
        """
        @Purpose:
        ...
        @Outputs:
            (list) positions.
        """
        (avg_array, std_array, ptv_array, raw_data) = self.samplesStats(self.samples, self.sampling_rate)
        if self.parameters.has_key('stdTolerance'):
            for i in range(len(self.channels)):
                if std_array[i] > self.parameters['stdTolerance']:
                    print "WARNING: channel#%d STD: %0.6f exceeded tolerance setting: %0.6f" % (i, std_array[i], self.parameters['stdTolerance'])
        retVal = avg_array * slope + piston
        return retVal
...
#        self.physicalChannel = "cDAQ9181-16FC143Mod1/ai" + str(self.ad_channel_number)
        ...
        returnValue = dll.DAQmxCreateTask(self.taskName, byref(self.taskHandle))
        returnValue = dll.DAQmxCreateAIVoltageChan(self.taskHandle, self.physicalChannel, self.channelName, self.terminalConfig, self.minVal, self.maxVal, self.units, self.customScaleName)
...
#        Define the parameters for the 9205 to obtain 1 sample which is read in a group.
    ...
    def samples_data(self, number, rates=0.0):
        ...
        if rates != 0.0:
            ...
            returnValue = dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(rates), DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, c_uint64(number))
        UINT_ARRAY = c_double*number
        sample_value = UINT_ARRAY()
        returnValue = dll.DAQmxReadAnalogF64(self.taskHandle, number, self.timeout, self.fillMode, sample_value, number, byref(self.sampsRead), self.reserved)
        ...
        if rates != 0.0:        # reset
            returnValue = dll.DAQmxCfgSampClkTiming(self.taskHandle, None, c_double(0.0), DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, c_uint64(number))
    ...
    def read_position(self, slope, piston):
        ...
        *** d = slope * voltage + piston ***
        ...
        returnValue = dll.DAQmxReadAnalogF64(self.taskHandle, self.samples, self.timeout, self.fillMode,self.sample_value, self.samples, byref(self.sampsRead), self.reserved)
        if self.sampsRead.value == 0:           # reading 0 samples, something's wrong
            raise CapgaugeException('returnValue = dll.DAQmxReadAnalogF64(self.taskHandle, self.samples, self.timeout, self.fillMode,self.sample_value, self.samples, byref(self.sampsRead), self.reserved)',
