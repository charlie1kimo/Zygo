...
int main (int argc, char **argv) {
  ...
  nInterfaces = PMD_Find(MCC_VID, USB1616FS_PID, fd);

  if ( nInterfaces <= 0 ) {
    fprintf(stderr, "USB 1616FS not found.\n");
    exit(1);
  } else
    printf("USB 1616FS Device is found! Number of Interfaces = %d\n",
	   nInterfaces);

  /* config mask 0x01 means all inputs */
  usbDConfigPort_USB1616FS(fd[0], DIO_DIR_OUT);
  usbDOut_USB1616FS(fd[0], 0);
  ...
  while(1) {
    ...
    switch(ch) {
      case 'b': /* test to see if led blinks */
        usbBlink_USB1616FS(fd[0]);
        break;
      case 'c':
        printf("connect pin 38 and 21\n");
        usbInitCounter_USB1616FS(fd[0]);
        sleep(1);
        flag = fcntl(fileno(stdin), F_GETFL);
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
        do {
          usbDOut_USB1616FS(fd[0], 1);
	  usleep(300000);
          usbDOut_USB1616FS(fd[0], 0);
	  printf("Counter = %d\n",usbReadCounter_USB1616FS(fd[0]));
        } while (!isalpha(getchar()));
        fcntl(fileno(stdin), F_SETFL, flag);
        break;
      case 'g':
        usbReadCode_USB1616FS(fd[0], 0x200000, 8, (__u8*) serial);
        serial[8] = '\0';
	...
	// Load the gain queue
	gainArray[0] = gain;
	usbAInLoadQueue_USB1616FS(fd[0], gainArray);
	...
	for ( i = 0; i < 1024; i++ ) {  // load data with known value
	  in_data[i] = 0xbeef;
        ...
        usbAInScan_USB1616FS(fd, 0, 0, count, &freq, options, in_data);
	...
	break;
      case 'i':
        ...
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
        do {
	  svalue = usbAIn_USB1616FS(fd[0], channel, gain);
	...
	break;
      case 's':
        printf("Status = %#x\n", usbGetStatus_USB1616FS(fd[0]));
	break;
      case 'r':
        usbReset_USB1616FS(fd[0]);
        return 0;
	break;
      case 'e':
        close(fd[0]);
