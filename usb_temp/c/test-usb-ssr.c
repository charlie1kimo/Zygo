...
int main (int argc, char **argv) {
 ...
 if ((nInterfaces = PMD_Find(MCC_VID, USBSSR24_PID, &fd)) > 0) {
   printf("USB SSR-24 Device is found! Number of Interfaces = %d\n", nInterfaces);
   device = USBSSR24_PID;
  } else if ((nInterfaces = PMD_Find(MCC_VID, USBSSR08_PID, &fd)) > 0) {
    printf("USB SSR-08 Device is found! Number of Interfaces = %d\n", nInterfaces);
    device = USBSSR08_PID;
  } else {
    fprintf(stderr, "USB SSR 24 & 08  not found.\n");
    exit(1);	
  }

  while(1) {
    printf("\nUSB SSR24 and SSR08 Testing\n");
    printf("----------------\n");
	  ...
	  if (port != 2 &&  port != 3) continue;
        ...
        switch (port) {
          case 0:   /* Port A */
            if (status & 0x1<<0) {
              printf("    Port A direction = input\n");
              input = usbDBitIn_USBSSR(fd, port, pin);
              printf("    Port %d  Pin %d = %#x\n", port, pin, input);
            } else {
              printf("    Port A direction = output\n");
              printf("Enter a bit value for output (0 | 1) : ");
	      scanf("%hhd", &bit_value);
              usbDBitOut_USBSSR(fd, port, pin, bit_value);
            }
            if (status & 0x1<<4) {
              printf("    Port A polarity = normal\n");
            } else {
