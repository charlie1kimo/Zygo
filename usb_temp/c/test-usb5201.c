...
int main (int argc, char **argv) {
  ...
  numInterfaces = PMD_Find(MCC_VID, USB5201_PID, fd);

  if ( numInterfaces <= 0 ) {
    fprintf(stderr, "USB-5201 not found.\n");
    exit(1);
  } else
    printf("USB-5201 Device is found! Number of interfaces = %d\n",
	   numInterfaces);
  ...
  while(1) {
	...
	case 'N':
	  ...
	  break;
        ...
        usbSetItem_USB5201(fd[0], i/2, i%2+CH_0_TC, bIReg);
        flag = fcntl(fileno(stdin), F_GETFL);
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
	do {
          usbTin_USB52XX(fd[0], i, 0, &temperature);
