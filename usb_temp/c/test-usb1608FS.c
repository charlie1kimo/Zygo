  ...
  while(1) {
    ...
    switch(ch) {
	  ...
	  printf("Counter = %d\n",usbReadCounter_USB1608FS(fd[0]));
        ...
        break;
      case 'g':
        usbReadCode_USB1608FS(fd[0], 0x200000, 8, (__u8*) serial);
        serial[8] = '\0';
	...
	for ( i = 0; i < 1024; i++ ) {  // load data with known value
	  in_data[i] = 0xbeef;
        ...
        usbAInScan_USB1608FS(fd, 0, 0, count, &freq, options, in_data, table_AIN, gainArray);
	...
	break;
      case 'i':
        ...
        fcntl(0, F_SETFL, flag | O_NONBLOCK);
        do {
	  svalue = usbAIn_USB1608FS(fd[0], channel, gain, table_AIN);
